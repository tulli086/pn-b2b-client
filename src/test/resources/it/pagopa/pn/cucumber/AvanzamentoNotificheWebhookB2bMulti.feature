Feature: avanzamento notifiche webhook b2b multi
  ...
  @clean @dev
  Scenario: [B2B-STREAM-TIMELINE_MULTI_5] Invio notifica digitale ed attesa elemento di timeline SCHEDULE_ANALOG_WORKFLOW_scenario positivo
    Given viene generata una nuova notifica
    | subject | invio notifica con cucumber |
    | senderDenomination | Comune di milano |
    And destinatario Mario Gherkin e:
    ...
    And si crea il nuovo stream per il "Comune_Multi"
    When la notifica viene inviata tramite api b2b dal "Comune_Multi" e si attende che lo stato diventi ACCEPTED
    Then vengono letti gli eventi dello stream del "Comune_Multi" fino all'elemento di timeline "SCHEDULE_ANALOG_WORKFLOW"

  @dev
  Scenario: [B2B-STREAM-TIMELINE_MULTI_6] Invio notifica digitale ed attesa elemento di timeline SEND_DIGITAL_PROGRESS_scenario positivo
    Given viene generata una nuova notifica
    | subject | invio notifica con cucumber |
    | senderDenomination | Comune di milano |
    And destinatario Mario Gherkin
  ...
  @dev
  Scenario: [B2B-STREAM-TIMELINE_MULTI_7] Invio notifica digitale ed attesa elemento di timeline SEND_DIGITAL_FEEDBACK_scenario positivo
    Given viene generata una nuova notifica
    | subject | invio notifica con cucumber |
    | senderDenomination | Comune di milano |
    And destinatario Mario Gherkin
