...
public class AsyncSteps {
  ...
  private String generateRandomIuv() {
    ...
    String numberOfThread =
        threadNumber.length() < 2 ? "0" + threadNumber : threadNumber.substring(0, 2);
  ...
  public void vieneCreataUnaPosizioneDebitoria(String organitationCode, String amount, String name,
      String taxId) {
    String iuv = generateRandomIuv();
    log.info("IUPD generate: " + organitationCode + "-64c8e41bfec846e04" + iuv,
    ...
    PaymentPositionModel paymentPositionModelSend = new PaymentPositionModel()
        .iupd(String.format(organitationCode + "-64c8e41bfec846e04" + iuv,
            System.currentTimeMillis()))
        ...
        .addPaymentOptionItem(new PaymentOptionModel()
            ...
            .retentionDate(
                new StringBuilder(dateTimeFormatter.format(OffsetDateTime.now().plusDays(2))))
    ...
    try {
      Assertions.assertDoesNotThrow(() -> {
        paymentPositionModel.add(
            pnGPDClientImpl.createPosition(organitationCode, paymentPositionModelSend, null, true));
      });
      ...
      String message = assertionFailedError.getMessage() +
          "{la posizione debitoria " + (paymentPositionModel == null ? "NULL"
          : paymentPositionModel.toString()) + " }";
  ...
  public void letturaAmountPosizioneDebitoria(String user) {
    ...
    PaymentInfoRequest paymentInfoRequest = new PaymentInfoRequest()
        .creditorTaxId(Objects.requireNonNull(
                Objects.requireNonNull(postionUser.getPaymentOption()).get(0).getTransfer()).get(0)
            .getOrganizationFiscalCode())
    ...
    for (int i = 0; i < NUM_CHECK_PAYMENT_INFO; i++) {
      try {
        ...
        String message = assertionFailedError.getMessage() +
            "{la posizione debitoria " + (paymentInfoResponse == null ? "NULL"
  ...
  public void vieneCancellataLaPosizioneDebitoria(String user) {
    try {
      for (PaymentPositionModel position : paymentPositionModel) {
        if (position.getFullName().equalsIgnoreCase(user)) {
          Assertions.assertDoesNotThrow(() -> {
            deleteGDPresponse = pnGPDClientImpl.deletePosition(Objects.requireNonNull(
                    Objects.requireNonNull(position.getPaymentOption()).get(0).getTransfer()).get(0)
                .getOrganizationFiscalCode(), position.getIupd(), null);
          });
        }
      }
  ...
  public void vieneCancellataLaPosizioneDebitoriaDelPagamento(Integer pagamento) {
    try {
      Assertions.assertDoesNotThrow(() -> {
        deleteGDPresponse = pnGPDClientImpl.deletePosition(Objects.requireNonNull(
                Objects.requireNonNull(paymentPositionModel.get(pagamento).getPaymentOption()).get(0)
                    .getTransfer()).get(0).getOrganizationFiscalCode(),
            paymentPositionModel.get(pagamento).getIupd(), null);
      });
      Assertions.assertNotNull(deleteGDPresponse);
      log.info("Risposta evento cancellazione: " + deleteGDPresponse);
  ...
  public void vengonoCancellateLaPosizioniDebitorie() {
    try {
      for (PaymentPositionModel paymentPositionModelUser : paymentPositionModel) {
        Assertions.assertDoesNotThrow(() -> {
          deleteGDPresponse = pnGPDClientImpl.deletePosition(Objects.requireNonNull(
                  Objects.requireNonNull(paymentPositionModelUser.getPaymentOption()).get(0)
                      .getTransfer()).get(0).getOrganizationFiscalCode(),
              paymentPositionModelUser.getIupd(), null);
        });
      }
      Assertions.assertNotNull(deleteGDPresponse);
      log.info("Risposta evento cancellazione: " + deleteGDPresponse);
  ...
  public void vieneEffettuatoIlControlloDelCambiamentoDelAmount(String timelineEventCategory,
      Integer user) {
    TimelineElementV23 timelineElement = sharedSteps.getTimelineElementByEventId(
        timelineEventCategory, null);
    int analogCost = Objects.requireNonNull(
        Objects.requireNonNull(timelineElement.getDetails()).getAnalogCost());
  ...
  public void vieneEffettuatoIlControlloDelCambiamentoDelAmountAlTentativo(
      String timelineEventCategory, Integer user, @Transpose DataTest dataFromTest) {
    TimelineElementV23 timelineElement = sharedSteps.getTimelineElementByEventId(
  ...
  public void vieneVerificatoIlCostoFinaleDellaNotificaAmount_gpdCosto_basePafeeAnalog_costPerOgniElementoDiTimeline() {
    ...
    String creditorTaxId = Assertions.assertDoesNotThrow(() -> Objects.requireNonNull(
        Objects.requireNonNull(sentNotification.getRecipients().get(0).getPayments()).get(0)
            .getPagoPa()).getCreditorTaxId());
    String noticeCode = Assertions.assertDoesNotThrow(() -> Objects.requireNonNull(
        Objects.requireNonNull(sentNotification.getRecipients().get(0).getPayments()).get(0)
    ...
    paymentInfoResponse = Assertions.assertDoesNotThrow(
  ...
  public void vieneAggiuntoIlCostoDellaNotificaTotaleAlUtente(Integer user) {
    try {
      for (int i = 0; i < amountNotifica.size(); i++) {
        Assertions.assertDoesNotThrow(() -> {
          int amount = Objects.requireNonNull(
              Objects.requireNonNull(sharedSteps.getSentNotification()).getAmount());
          int paFee = Objects.requireNonNull(
              Objects.requireNonNull(sharedSteps.getSentNotification()).getPaFee());
  ...
  public void vieneEffettuatoIlControlloDelAmountDiGPDConCostoTotaleConIva(String tipoCosto) {
    try {
      log.info("Amount GPD: " + amountGPD);
      amountGPD = amountGPD - Integer.parseInt(String.valueOf(
          Objects.requireNonNull(paymentPositionModel.get(0).getPaymentOption()).get(0)
              .getAmount()));
  ...
  private Integer getCostoTotale(FullSentNotificationV23 sentNotification) {
    ...
    for (TimelineElementV23 timelineElem : sentNotification.getTimeline()) {
      Integer currentCost = timelineElem.getDetails() == null ? Integer.valueOf(0)
          : timelineElem.getDetails().getAnalogCost();
        if (currentCost != null && currentCost > 0) {
            analogCost += currentCost;
        }
    }
