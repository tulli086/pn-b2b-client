...
public class RaddAltSteps {
  ...
  public void lOperatoreUsoIUNPerRecuperariGliAtti(String tipologiaIun, String cf) {
    ...
    ActInquiryResponse actInquiryResponse = raddAltClient.actInquiry(uid,
        ...
        tipologiaIun.equalsIgnoreCase("corretto") ? this.iun = sharedSteps.getIunVersionamento() :
            tipologiaIun.equalsIgnoreCase("errato") ? this.iun = "GLDZ-MGZD-AGAR-202402-Y-1"
                : null);
  ...
  public void lOperatoreUsoIUNPerRecuperariGliAttiDaIssuer(String tipologiaIun, String cf,
      String issuer) {
  ...
  public void vieneRestituitoUnMessaggioDiErrore(String errorType, int errorCode) {
    ...
    switch (errorType) {
      ...
      case "stampa già eseguita", "questa notifica è stata annullata dall’ente mittente", "documenti non più disponibili", "ko generico", "input non valido" -> {
        ...
        Assertions.assertEquals(errorType.toLowerCase(),
            actInquiryResponse.getStatus().getMessage().toLowerCase());
        Assertions.assertEquals(error, actInquiryResponse.getStatus().getCode());
  ...
  public void laScansioneSiConcludeCorrettamenteAlternative() {
    ...
    Assertions.assertNotNull(actInquiryResponse.getStatus());
    Assertions.assertEquals(ActInquiryResponseStatus.CodeEnum.NUMBER_0,
        actInquiryResponse.getStatus().getCode());
  }
  ...
  private void uploadDocumentRaddAlternative(boolean usePresignedUrl) {
    try {
      creazioneZip();
      PnPaB2bUtils.Pair<String, String> uploadResponse = pnPaB2bUtils.preloadRaddAlternativeDocument(
          "classpath:/" + this.fileZip, usePresignedUrl, this.operationid);
  ...
  private void startTransactionActRaddAlternative(String operationid) {
    ActStartTransactionRequest actStartTransactionRequest =
        new ActStartTransactionRequest()
            ...
            .recipientType(this.recipientType.equalsIgnoreCase("PF")
                ? ActStartTransactionRequest.RecipientTypeEnum.PF :
    ...
    if (this.startTransactionResponse.getStatus().getCode()
        .equals(StartTransactionResponseStatus.CodeEnum.NUMBER_2)) {
  ...
  public void lOperazioneDiDownloadDegliAttiSiConcludeCorrettamente() {
    ...
    Assertions.assertEquals(StartTransactionResponseStatus.CodeEnum.NUMBER_0,
        this.startTransactionResponse.getStatus().getCode());
  }
  ...
  public void lOperazioneDiDownloadDegliAttiGeneraUnErroreConCodice(String errorDescription,
      int erroCode) {
    ...
    Assertions.assertEquals(errorDescription.trim().toLowerCase(),
        this.startTransactionResponse.getStatus().getMessage().toLowerCase());
  }
  ...
  public void laVerificaAorMostraCorrettamenteLeNotificheInStatoIrreperibile() {
    ...
    Assertions.assertEquals(ResponseStatus.CodeEnum.NUMBER_0,
        this.aorInquiryResponse.getStatus().getCode());
  ...
  public void vengonoRecuperatiGliAttiDelleNotificheInStatoIrreperibile() {
    AorStartTransactionRequest aorStartTransactionRequest =
        new AorStartTransactionRequest()
            ...
            .recipientTaxId(this.currentUserCf)
            .recipientType(this.recipientType.equalsIgnoreCase("PF")
                ? AorStartTransactionRequest.RecipientTypeEnum.PF :
  ...
  public void vengonoRecuperatiGliAttiDelleNotificheInStatoIrreperibileStessoOperationId(
  ...
  public void ilRecuperoDegliAttiInStatoIrreperibileSiConcludeCorrettamente() {
    ...
    Assertions.assertEquals(StartTransactionResponseStatus.CodeEnum.NUMBER_0,
        this.aorStartTransactionResponse.getStatus().getCode());
  }
  ...
  public void ilRecuperoDegliAarInStatoIrreperibileSiConcludeCorrettamenteEVengonoRestituitiTuttiEGliAar(
      int aarNumber) {
    log.info("aorStartTransactionResponse: {}", this.aorStartTransactionResponse);

    Assertions.assertNotNull(this.aorStartTransactionResponse.getDownloadUrlList());
    Assertions.assertEquals(this.aorStartTransactionResponse.getDownloadUrlList().size(),
        aarNumber);
    Assertions.assertFalse(this.aorStartTransactionResponse.getDownloadUrlList().isEmpty());
    Assertions.assertNotNull(this.aorStartTransactionResponse.getStatus());
    Assertions.assertEquals(StartTransactionResponseStatus.CodeEnum.NUMBER_0,
  ...
  public void ilRecuperoDegliAarGeneraUnErroreConCodice(String errorType, int errorCode) {
    ...
    Assertions.assertEquals(errorType,
        this.aorStartTransactionResponse.getStatus().getMessage().toLowerCase());
  }
  ...
  public void laVerificaDellaPresenzaDiNotificheInStatoIrreperibiGeneraUnErroreConCodice(
  ...
  public void laChiusuraDelleTransazionePerIlRecuperoDegliAarNonGeneraErrori() {
    ...
    Assertions.assertEquals(TransactionResponseStatus.CodeEnum.NUMBER_0,
        this.completeTransactionResponse.getStatus().getCode());
  }
  ...
  public void laChiusuraDelleTransazionePerIlRecuperoDegliAarNonGeneraErrori(String error,
      int statusCode) {
    Assertions.assertNotNull(this.completeTransactionResponse);
    Assertions.assertNotNull(this.completeTransactionResponse.getStatus());
    Assertions.assertNotNull(this.completeTransactionResponse.getStatus().getCode());
    Assertions.assertEquals(new BigDecimal(statusCode),
        this.completeTransactionResponse.getStatus().getCode().getValue());
  ...
  public void vengonoCaricatiIDocumentoDiIdentitàDelCittadinoSenza(String without) {
    ...
    documentUploadRequest = without.equalsIgnoreCase("contentType") ? documentUploadRequest
        : documentUploadRequest.checksum(sha256);

    try {
      DocumentUploadResponse documentUploadResponse = raddAltClient.documentUpload(this.uid,
  ...
  public void lOperazioneDiAbortGeneraUnErroreConCodice(String error, int statusCode) {
    ...
    Assertions.assertEquals(new BigDecimal(statusCode),
        this.abortActTransaction.getStatus().getCode().getValue());
  ...
  public void ilCittadinoMostraIlQRCodeRaddAlternative(String cf, Integer destinatario,
      String qrCodeType) {
    selectUserRaddAlternative(cf);
    ...
    switch (qrCodeType) {
      ...
      case "appartenente a terzo" -> {
        if (this.currentUserCf.equalsIgnoreCase(
  ...
  public void lOperatoreScansioneIlQrCodePerRecuperariGliAtti() {
    ActInquiryResponse actInquiryResponse = raddAltClient.actInquiry(uid, this.currentUserCf,
        this.recipientType, qrCode, null);
    log.info("actInquiryResponse: {}", actInquiryResponse);
    this.actInquiryResponse = actInquiryResponse;
  }
  ...
  public void lOperatoreEsegueDownloadFrontespizio(String operationType) {
    ...
    pnPaB2bUtils.stampaPdfTramiteByte(download,
        "target/classes/frontespizio" + generateRandomNumber() + ".pdf");
  }
  ...
  public void changeRaddista(String raddista) {
    switch (raddista.toLowerCase()) {
      ...
      case "issuer_non_censito" -> raddAltClient.setAuthTokenRadd(
          SettableAuthTokenRadd.AuthTokenRaddType.ISSUER_NON_CENSITO);
